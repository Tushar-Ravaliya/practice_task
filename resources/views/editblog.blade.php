                    ...
                    <form class="form-card" action="{{ URL::to('/') }}/blogs/{{ $blogs['id'] }}" method="POST"
                        ...
                        <div class="row justify-content-end  text-left ">
                            <div class="form-group  flex-column d-flex "> <label
                                ...
                                <input type="text" name="title" placeholder="Enter your first name"
                                    value="{{ $blogs->title }}">
                            </div>
                            @error('title')
                            ...
                            <div class="form-group col-12 flex-column d-flex"> <label
                                ...
                                <textarea type="text" id="ans" name="description" placeholder="">{{ $blogs->description }} </textarea>
                            </div>
                            @error('description')
                                <span style="color: red;">{{ $message }}</span>
                            @enderror
                            <div class="image_box">
                                <div class="form-group  flex-column d-flex image"> <label
                                    ...
                                    <input type="file" id="images" name="blog_images[]"accept="image/*"
                                        value="{{ old('blog_images[]') }}" onchange="preview_image(event)" multiple>
                                </div>
                            </div>
                            @error('blog_images[]')
                                <span style="color: red;">{{ $message }}</span>
                        ...
                        <div id="preview" class="d-flex justify-content-around flex-wrap">
                            @if ($blogs->images->count() > 0)
                        ...
                        <div class="row justify-content-between text-left links_box" id='links_box'>
                            @if ($blogs->links->count() > 0)
                                @foreach ($blogs->links as $link)
                                    <div class="form-group col-sm-3  flex-column d-flex  title" id='title'> <label
                                            class="form-control-label px-3 ">Title<span
                                                class="text-danger">*</span></label>
                                        <input type="text" id="fname"
                                            name="blog_title[]"placeholder="Enter your link name"
                                    ...
                                    <div class="form-group col-sm-9  flex-column d-flex  links" id='links'>
                                        <label class="form-control-label px-3 ">Links<span
                                                class="text-danger">*</span></label>
                                        <input type="text" id="fname" name="blog_link[]"
                                            value="{{ $link->links }}" placeholder="Enter your link">
                                    </div>
                                    @error('blog_link')
                                        <span style="color: red;">{{ $message }}</span>
                                    @enderror
                        ...
                        <div class="row justify-content-end">
                            <div class="form-group col-sm-6"> <button type="submit" class="text-white bg-primary">Edit
                                    Blog</button> </div>
                        </div>
                    </form>
                </div>
